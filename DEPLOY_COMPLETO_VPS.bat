...
echo.
ssh -t administrator@155.117.47.244 "sudo bash -c 'cd /root/imperio-crm/evo-crm-clovisvallejr_2 2>/dev/null || cd /root/evo-crm-community 2>/dev/null && sed -i \"s|^FRONTEND_URL=.*|FRONTEND_URL=https://imperiocrm.com.br|\" .env && sed -i \"s|^BACKEND_URL=.*|BACKEND_URL=https://imperiocrm.com.br/crm|\" .env && git pull origin main && docker compose build && docker compose run --rm evo-crm bundle exec rails db:migrate && docker compose run --rm evo-auth bundle exec rails db:seed && docker compose up -d'"
