...
class Product < ApplicationRecord
  ...
  def validate_images_count
    if images.attached? && images.length > 5
      errors.add(:images, 'pode ter no máximo 5 imagens anexadas')
    end
  end
  ...
  def to_prompt_summary
    image_url = nil
    if images.attached?
      image_url = Rails.application.routes.url_helpers.rails_blob_url(images.first, host: Rails.application.routes.default_url_options[:host] || ENV.fetch('BACKEND_URL', 'http://localhost:3000'))
    end
