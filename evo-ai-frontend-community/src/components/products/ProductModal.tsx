...
export default function ProductModal({ open, product, loading, onOpenChange, onSubmit }: Props) {
  ...
  useEffect(() => {
    ...
    if (product) {
      ...
      setCoverImageId(product.metadata?.cover_image_id as string | number | null ?? null);
    } else {
      setForm(emptyForm());
      setVariants([]);
      setLabelsText('');
  ...
  const handleFilesSelected = (files: FileList | null) => {
    if (!files) return;
    setNewFiles((prev) => {
      const existingCount = (product?.images?.length ?? 0) - deletedImageIds.length;
      const totalCount = existingCount + prev.length + files.length;
      if (totalCount > 5) {
        toast.error(t('Atenção: O limite é de 5 imagens por produto.'));
        const allowedFiles = Array.from(files).slice(0, 5 - existingCount - prev.length);
  ...
  const handleSubmit = async () => {
    ...
    const payload: ProductFormData = {
      ...form,
      labels,
      metadata: { ...form.metadata, cover_image_id: coverImageId },
    ...
    >
      <Tabs defaultValue="general" className="flex-1 overflow-hidden flex flex-col">
        ...
        <TabsContent value="media" className="space-y-3 overflow-y-auto pt-4">
          ...
          {product?.images && product.images.filter(img => !deletedImageIds.includes(String(img.id))).length > 0 && (
            <div>
              <p className="text-sm font-medium mb-2">{t('media.existing')}</p>
              <div className="grid grid-cols-3 gap-3">
                {product.images.filter(img => !deletedImageIds.includes(String(img.id))).map((image) => (
                  <div 
                    key={image.id} 
                    className={`border rounded overflow-hidden relative cursor-pointer group ${coverImageId === image.id ? 'ring-2 ring-primary border-transparent' : ''}`}
                    onClick={() => setCoverImageId(image.id)}
                    title="Clique para definir como capa"
                  >
                    <img src={getImageUrl(image.url)} alt={image.filename} className="w-full h-32 object-cover" />
                    <div className="px-2 py-1 text-xs truncate flex justify-between items-center">
                      <span className="truncate">{image.filename}</span>
                      {coverImageId === image.id && <span className="text-[10px] font-bold text-primary ml-1 uppercase">Capa</span>}
                    </div>
                    <Button
                      variant="destructive"
                      size="icon"
