      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {products.map((product) => {
            ...
            const coverImageId = product.metadata?.cover_image_id;
            let displayImage = product.images?.find(i => String(i.id) === String(coverImageId));
            ...
            return (
              <tr key={product.id} className="border-t hover:bg-muted/30">
                <td className="px-3 py-2">
                  <div className="h-10 w-10 rounded overflow-hidden bg-muted flex items-center justify-center border shrink-0">
                    {displayImage ? (
                      <img src={getImageUrl(displayImage.url)} alt={product.name} className="w-full h-full object-cover" />
                    ) : (
