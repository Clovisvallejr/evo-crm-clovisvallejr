...
class ProductsService {
  ...
  async updateProduct(id: string, payload: Partial<ProductFormData>, files?: File[]): Promise<Product> {
    ...
    const hasDeletedImages = payload.deleted_image_ids && payload.deleted_image_ids.length > 0;
  ...
  private buildFormData(payload: Partial<ProductFormData>, files: File[]): FormData {
    ...
    Object.entries(payload).forEach(([key, value]) => {
      ...
      if (key === 'deleted_image_ids' && Array.isArray(value)) {
        value.forEach((id) => formData.append('product[deleted_image_ids][]', String(id)));
        return;
      }
