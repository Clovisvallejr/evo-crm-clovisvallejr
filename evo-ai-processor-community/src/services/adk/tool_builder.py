    ...
    def build_tools(
        ...
        if enable_crm_tools:
            ...
            try:
                ...
                # Always add our new CRM tools if crm tools are enabled
                try:
                    
                    quote_tool = create_quote_tool()
                    self.tools.append(quote_tool)
                    logger.info("Added create_quote_tool from CRM tools")

                    order_tool = create_order_tool()
                    self.tools.append(order_tool)
                    logger.info("Added create_order_tool from CRM tools")

                except Exception as inner_e:
                    logger.error(f"Error loading new custom CRM tools: {inner_e}")
