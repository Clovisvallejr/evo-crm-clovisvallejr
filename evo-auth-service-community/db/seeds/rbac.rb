...
agent_permissions = [
  ...
  'journeys.read',
  'campaigns.read',
  'orders.read', 'orders.create', 'orders.update', 'orders.delete', 'orders.bulk_action',
...
salesperson = Role.find_or_initialize_by(key: 'salesperson')
if salesperson.new_record?
  salesperson.name = 'Vendedor'
  salesperson.description = 'Vendedor com acesso a orçamentos, produtos e contatos'
  salesperson.system = true
  salesperson.type = 'account'
  salesperson.save!
  puts "   ✅ Created role: #{salesperson.name}"
else
  if salesperson.type != 'account'
    salesperson.update!(type: 'account')
  end
end

salesperson_permissions = agent_permissions + [
  'products.read'
]

salesperson.role_permissions_actions.destroy_all
salesperson_permissions.select { |key| ResourceActionsConfig.valid_permission?(key) }.uniq.each do |permission_key|
  salesperson.role_permissions_actions.create!(permission_key: permission_key)
end

# Sales Manager (Gerente de Vendas)
sales_manager = Role.find_or_initialize_by(key: 'sales_manager')
if sales_manager.new_record?
  sales_manager.name = 'Gerente de Vendas'
  sales_manager.description = 'Gerente com acesso logístico e relatórios'
  sales_manager.system = true
  sales_manager.type = 'account'
  sales_manager.save!
  puts "   ✅ Created role: #{sales_manager.name}"
else
  if sales_manager.type != 'account'
    sales_manager.update!(type: 'account')
  end
end

sales_manager_permissions = salesperson_permissions + [
  'orders.update_logistics',
  'reports.read',
  'live_reports.read',
  'summary_reports.read',
  'pipelines.read',
  'pipelines.update'
]

sales_manager.role_permissions_actions.destroy_all
sales_manager_permissions.select { |key| ResourceActionsConfig.valid_permission?(key) }.uniq.each do |permission_key|
  sales_manager.role_permissions_actions.create!(permission_key: permission_key)
end
puts "   📋 Assigned #{sales_manager_permissions.select { |key| ResourceActionsConfig.valid_permission?(key) }.size} permissions to #{sales_manager.name}"
